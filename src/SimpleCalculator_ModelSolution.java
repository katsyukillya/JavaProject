...
/**
 * @author John Doe Description: This application gives choice to add, subtract,
 *         multiply and divide two numbers. After successful calculation it lets
 *         user calculate again or exit the system
 *
 */
public class SimpleCalculator_ModelSolution {
	/**
	...
	/**
	 * Subtracts two numbers passed as two elements of an array and prints the
	 * result
	 * 
	 * @param array
	 *            of two floats
	...
	/**
	 * Multiplies two numbers passed as two elements of an array and prints the
	 * result
	 * 
	 * @param array
	 *            of two floats
	...
	public static float[] getTwoFloats(int calcOption, Scanner readInput) {
		...
		do {
			String calcOperation = (calcOption == 1) ? "add"
					: (calcOption == 2) ? "subtract" : (calcOption == 3) ? "multiply" : "divide";
			...
			if (readInput.hasNextFloat()) {
				...
				if (readInput.hasNextFloat()) {
					...
					if (floatNumbers[1] == 0 && calcOperation.equalsIgnoreCase("divide"))
